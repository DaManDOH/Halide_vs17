...
int main(int argc, char **argv) {
    ...
    if (!f || fread(header, 32, 1, f) != 1) {
        std::cout << "Object file not generated\n";
        return -1;
    }
    fclose(f);

    if (memcmp(header, arm_32_android_magic, sizeof(arm_32_android_magic))) {
        std::cout << "Unexpected header bytes in 32-bit arm object file.\n";
        return -1;
    }
    ...
    if (memcmp(header, win_64_magic, sizeof(win_64_magic))) {
        std::cout << "Unexpected header bytes in 64-bit windows object file.\n";
        return -1;
    }
    ...
    if (memcmp(header, arm_32_ios_magic, sizeof(arm_32_ios_magic))) {
        std::cout << "Unexpected header bytes in 32-bit arm ios object file.\n";
        return -1;
    }
