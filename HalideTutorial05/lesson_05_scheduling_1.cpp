...
int main(int argc, char **argv) {
	...
	// First we observe the default ordering.
	{
		...
		for (int y = 0; y < 4; y++) {
			for (int x = 0; x < 4; x++) {
				std::cout << "Evaluating at x = " << x << ", y = " << y << ": " << (x + y) << "\n";
			}
		}
		std::cout << "\n\n";
	...
	// Evaluating in tiles.
	{
		...
		std::cout << "Evaluating gradient in 4x4 tiles\n";
	...
	// Evaluating in vectors.
	{
		...
		std::cout << "Evaluating gradient with x_inner vectorized \n";
		...
		for (int y = 0; y < 4; y++) {
			for (int x_outer = 0; x_outer < 2; x_outer++) {
				...
				int val[] = { x_vec[0] + y,
							 ...
							 x_vec[2] + y,
							 x_vec[3] + y };
				std::cout << "Evaluating at <" << x_vec[0] << ", " << x_vec[1] << ", " << x_vec[2] << ", " << x_vec[3] << ">, "
					<< "<" << y << ", " << y << ", " << y << ", " << y << ">: "
	...
	// Unrolling a loop.
	{
		...
		std::cout << "Evaluating gradient unrolled by a factor of two\n";
	...
	// Splitting by factors that don't divide the extent.
	{
		...
		std::cout << "Evaluating gradient over a 7x2 box with x split by three \n";
	...
	// Fusing, tiling, and parallelizing.
	{
		...
		std::cout << "Evaluating gradient tiles in parallel\n";
		Buffer<int> output = gradient.realize(8, 8);
	...
	// Putting it all together.
	{
		...
		std::cout << "Checking Halide result against equivalent C...\n";
					...
					int y_base = std::min(y_outer * 64, 250 - 64) + y_inner_outer * 2;
					{
						...
						for (int i = 0; i < 4; i++) {
							if (result(x_vec[i], y_vec[i]) != val[i]) {
								std::cout << "There was an error at " << x_vec[i] << " " << y_vec[i] << "!\n";
								return -1;
							}
						}
					}
