...
// Suppress Visual Studio unsafe function warnings.
#ifdef _MSC_VER
...
int main(int argc, char **argv) {
	std::string targetFilename = "images/gray.png";
	if (argc > 1) {
	...
	Buffer<uint8_t> input = load_image(targetFilename);
		...
		for (int y = 0; y < 100; y++) {
			for (int x = 0; x < 100; x++) {
				if (fabs(halide_result(x, y) - c_result[y][x]) > 0.01f) {
					std::cout << "halide_result(" << x << ", " << y << ") = " << halide_result(x, y) << " instead of " << c_result[y][x] << "\n";
					return -1;
				}
			}
		}
	...
	// computing a histogram.
	{
		...
		for (int x = 0; x < 256; x++) {
			if (c_result[x] != halide_result(x)) {
				std::cout << "halide_result(" << x << ") = " << halide_result(x) << " instead of " << c_result[x] << "\n";
				return -1;
			}
		}
	}
	...
	{
		{
			...
			for (int x = 0; x < 10; x++) {
				if (halide_result(x) != c_result[x]) {
					std::cout << "halide_result(%d) = %d instead of %d\n", x, halide_result(x), c_result[x];
					return -1;
				}
			}
		}
...

		{
			...
			for (int y = 0; y < 10; y++) {
				for (int x = 0; x < 10; x++) {
					if (halide_result(x, y) != c_result[y][x]) {
						std::cout << "halide_result(%d, %d) = %d instead of %d\n", x, y, halide_result(x, y), c_result[y][x];
						return -1;
					}
				}
			}
...
#pragma omp parallel for
		...
		std::cout << "Halide spread took %f ms. C equivalent took %f ms\n", (t3 - t2) / 100, (t2 - t1) / 100;
